...
def conv(batchNorm, cin, cout, k=3, stride=1, pad=-1):
    ...
    #if batchNorm: /////////////////////////////////// all the conv layers have batch normalization //////////////////////////////////////
    ...
    """
###########################################################################################
###################  Convolutionel layer with batch Norm and NO LReLU Func ################
def convS(batchNorm, cin, cout, k=3, stride=1, pad=1):
    pad = (k - 1) // 2 if pad < 0 else pad
    print('=> convolutional layer with bachnorm without LReLU')
    return nn.Sequential(
                nn.Conv2d(cin, cout, kernel_size=k, stride=stride, padding=pad, bias=False),
                nn.BatchNorm2d(cout),
                )
...
##########################################################################################
########################## concatination and calcule error witn LReLU Function############
def LReLU1():
    return nn.Sequential(
                nn.LeakyReLU(0.1, inplace=True)
                )